...
MCP Atlassian provides **76 tools** for interacting with Jira and Confluence. Tools are organized by category below.
...
| `jira_comments` | Yes | `jira_add_comment`, `jira_edit_comment`, `jira_delete_comment` |
...
| `jira_users` | No | `jira_get_current_user`, `jira_get_user_profile` |
...
| `confluence_users` | No | `confluence_get_current_user`, `confluence_search_user` |
