...
| `ATLASSIAN_IDENTITY_GUARD_USER` | Explicit expected Atlassian user for shared-agent setups (account ID, email, username, or display name) |
| `ATLASSIAN_IDENTITY_GUARD_ACCOUNT_ID` | Explicit expected Atlassian account ID (preferred for Cloud) |
| `ATLASSIAN_IDENTITY_GUARD_EMAIL` | Explicit expected Atlassian email fallback |
...
# Optional: block writes unless the request supplies its own Atlassian auth
