...
| `ATLASSIAN_IDENTITY_GUARD_MODE` | Verify authenticated Atlassian user before writes (`off`, `write`, `all`) |
| `ATLASSIAN_IDENTITY_GUARD_USER` | Expected Atlassian user for single-user/local stdio setups only. For shared HTTP servers, send `X-Atlassian-Expected-User` from each local client instead. |
| `ATLASSIAN_IDENTITY_GUARD_ACCOUNT_ID` | Expected Atlassian account ID for single-user/local stdio setups only (preferred for Cloud) |
| `ATLASSIAN_IDENTITY_GUARD_EMAIL` | Expected Atlassian email fallback for single-user/local stdio setups only |
| `ATLASSIAN_IDENTITY_GUARD_REQUIRE_REQUEST_AUTH` | Disable global credential fallback while identity guard is enabled (`true`/`false`) |
...
# Identity guard for shared Codex/agent accounts.
# On a shared HTTP server, do not set ATLASSIAN_IDENTITY_GUARD_USER here.
# Each local client must send X-Atlassian-Expected-User instead.
ATLASSIAN_IDENTITY_GUARD_MODE=write
ATLASSIAN_IDENTITY_GUARD_REQUIRE_REQUEST_AUTH=true
```

Local HTTP client configuration for one user:
...
```json
{
  "mcpServers": {
    "mcp-atlassian": {
      ...
      "headers": {
        "X-Atlassian-Expected-User": "andrej@example.com"
      }
    }
  }
}
...
For local stdio setups where each user runs their own MCP process, the expected
identity can be set as a local environment variable instead:

```bash
ATLASSIAN_IDENTITY_GUARD_USER=andrej@example.com
```
